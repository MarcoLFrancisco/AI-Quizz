...
(() => {
  ...
  if (!Array.isArray(questions) || questions.length !== 8) {
    status.textContent = "Question data is unavailable. Check questions.js and reload.";
    return;
  }
  ...
  function renderQuestion() {
    ...
    app.innerHTML = `
      <div class="progress-meta">
        ...
        <span id="points">${score()} points earned</span>
      </div>
      <progress value="${index}" max="${questions.length}" aria-label="Questions completed"></progress>
      <span class="badge ${multiple ? "multiple" : ""}">${multiple ? "Multi-answer · Choose 2" : "Single answer"}</span>
      <h2 id="question-title" tabindex="-1" data-heading>${escape(question.title)}</h2>
      <p id="instruction" class="instruction muted">${multiple ? "Select exactly two answers. Both must be correct to earn the point." : "Choose one answer."}</p>
      <form novalidate>
        ...
        <p id="selection-count" class="instruction muted" aria-live="polite" ${multiple ? "" : "hidden"}>0 of 2 answers selected</p>
        <p id="error" class="error" role="alert"></p>
        <div id="feedback" hidden tabindex="-1" role="region" aria-labelledby="feedback-title"></div>
        <div class="actions"><button class="primary" type="submit">Check answer</button></div>
      </form>`;
    ...
    form.addEventListener("change", () => {
      error.textContent = "";
      if (multiple) app.querySelector("#selection-count").textContent = `${selectedInputs(form).length} of 2 answers selected`;
    });
    form.addEventListener("submit", (event) => {
      ...
      if (selected.length !== question.correct.length) {
        error.textContent = multiple ? "Please choose exactly two answers." : "Please choose an answer.";
        return;
      }
      checked = true;
      ...
      form.querySelectorAll("input").forEach((input) => {
        ...
        tag.textContent = isAnswer
          ? `✓ Correct answer · ${input.checked ? "Selected" : "Not selected"}`
          : "✕ Your selection · Incorrect";
        label.querySelector(".option-text").appendChild(tag);
      });
      app.querySelector("#points").textContent = `${score()} points earned`;
      app.querySelector("progress").value = index + 1;
      const feedback = app.querySelector("#feedback");
      feedback.className = `feedback ${correct ? "" : "wrong"}`;
      feedback.innerHTML = `<strong id="feedback-title">${correct ? "✓ Nicely focused! +1 point" : "Not quite. Here’s the takeaway."}</strong><p>${escape(question.explanation)}</p>`;
      feedback.hidden = false;
      button.type = "button";
      button.textContent = index === questions.length - 1 ? "See my score →" : "Next question →";
  ...
  function renderResults() {
    ...
    const title = total === questions.length ? "Picture perfect!" : percentage >= 75 ? "Looking sharp!" : "Coming into focus!";
    app.innerHTML = `
      <div class="result-top">
        <p class="eyebrow">Quiz complete</p>
        <div class="score-ring" style="--score-angle: ${percentage * 3.6}deg" aria-hidden="true">
          <div class="score-center"><strong>${percentage}%</strong><span>Final score</span></div>
        </div>
        <h2 tabindex="-1" data-heading>${title}</h2>
        <p>You scored <strong>${total} out of ${questions.length}</strong> (${percentage}%).</p>
        <p class="muted">Review your answers below, then take another shot. Keep checking important AI suggestions before using them.</p>
      </div>
      <div class="review">${questions.map((question, questionIndex) => {
        ...
        return `<details>
          <summary><span class="review-status ${response.correct ? "" : "wrong"}">${response.correct ? "✓ Correct" : "✕ Incorrect"}</span> · ${questionIndex + 1}. ${escape(question.title)}</summary>
          <p><strong>Your answer:</strong> ${answerText(response.selected)}</p>
          <p><strong>Correct answer:</strong> ${answerText(question.correct)}</p>
          <p class="muted">${escape(question.explanation)}</p>
        </details>`;
      }).join("")}</div>
      <button id="restart" class="primary" type="button">Take another shot ↻</button>`;
